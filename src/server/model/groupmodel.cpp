...
bool GroupModel::creatGroup(Group& group)
{
    ...
    MySQL mysql;
    if (mysql.connect()) // 说明连接上了数据库
    {
        if (mysql.update(sql)) // 如果插入数据成功了
        {
            // 从该对象获取群组id
            // 传入的参数是MYSQL类型的指针 通过getconnection方法得到
...
vector<Group> GroupModel::queryGroups(int userid)
{
    ...
    // 根据 userid -> 所有的 groupid -> 每一个群组的详细信息
    char sql[1024] = {0};
    ...
    if (mysql.connect())
    {
        MYSQL_RES* res = mysql.query(sql);  // 得到 mysql result
        if (res != nullptr)
        {
            MYSQL_ROW row;
